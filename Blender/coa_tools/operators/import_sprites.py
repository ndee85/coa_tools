        ...
        if "nodes" in sprite_data:
            for i,sprite in enumerate(sprite_data["nodes"]):
                if sprite["name"] in self.json_import_data and self.json_import_data[sprite["name"]].active:
                    ...
                    svgfilepath = sprite.get("vector_path", "")
                    if svgfilepath:
                        svgfilepath = os.path.join(folder, svgfilepath)
                    ...
                    if self.mode == "ADD":
                        bpy.ops.coa_tools.coa_import_sprite(path=filepath,svgpath=svgfilepath,parent=sprite_object.name,scale=scale,pos=pos,tilesize=tilesize,offset=offset)
                    if self.mode == "UPDATE":
...
class ImportSprite(bpy.types.Operator):
    ...
    svgpath = StringProperty(name="Sprite SVG Path", default="",subtype="FILE_PATH")
    ...
    def join_objects(self, obj, selected):
        verts = obj.data.copy().vertices
        bpy.context.scene.objects[selected.name].select = True
        obj.select = True
        ...
        bpy.ops.object.coa_fill(triangulate=True)
        bpy.ops.object.mode_set(mode="OBJECT")
    ...
    def create_svg_mesh(self, context):
        #import svg as a curve
        objects = set([obj.name for obj in context.scene.objects])
        bpy.ops.import_curve.svg(filepath=self.svgpath)
        objects_after = set([obj.name for obj in context.scene.objects])
        obj_created = list(objects_after - objects)[0]

        #convert curve to mesh
        curve = bpy.data.objects[obj_created]
        scale = get_addon_prefs(context).sprite_import_export_scale
        svgscale = 11800 * scale
        mesh_from_curve = curve.to_mesh(context.scene, False, 'PREVIEW')

        obj = bpy.data.objects.new(curve.name + "Mesh", mesh_from_curve)
        obj.rotation_euler = (radians(90), 0, 0)
        obj.dimensions = curve.dimensions * svgscale
        obj.location = Vector((0,self.pos[1],0))*self.scale + Vector((self.offset[0],self.offset[1],self.offset[2]))*self.scale
        obj.parent = bpy.data.objects[self.parent]

        context.scene.objects.link(obj)
        context.scene.objects.unlink(curve)

        self.extrude_obj(obj)
        self.decimate_obj(obj)
        self.remove_extrude_obj(obj)

        return obj

    def extrude_obj(self, obj):
        mesh = bmesh.new()
        mesh.from_mesh(obj.data)

        top = bmesh.ops.extrude_edge_only(mesh, edges=mesh.edges)
    ...
    def decimate_obj(self, obj):
        ...
        #TODO: allow setting decimate ratio
        modifier.ratio = .1
    ...
    def remove_extrude_obj(self, obj):
        mesh = bmesh.new()
        mesh.from_mesh(obj.data)

        verts = [v for v in mesh.verts if v.co[2] > 0.0]
    ...
    def execute(self,context):
        if os.path.exists(self.path):
            ...
            if self.svgpath:
                created_obj = self.create_svg_mesh(context)
                self.join_objects(obj, created_obj)
